...
compatibility: Requires the Bend 2 CLI. Detailed claims were checked with Bend 2.0.10; trust the installed `bend guide` and `bend base <Name>` when versions differ.
---
...
- If the toolchain is unknown, run `bend --version`. Stop and report if Bend
  is missing or not Bend 2.
- Use targeted lookup: `bend base <Name>` or `bend base --types`. Bare
  `bend base` is large.
- Run or search `bend guide` when syntax is unfamiliar, the installed version
  changed, or this skill conflicts with the checker. For graphics, use
  `bend guide shaders`.
...
- Recursion must structurally decrease. Arguments are checked left to right:
  ...
  decreasing argument as early as practical in a new API. No mutual recursion.
...
- Bend does almost no inference. Annotate when the intended type is not clear.
  Unannotated arithmetic is valid `Nat`; use `(a + b : U32)` for U32 arithmetic.
...
- A parallel let promises independent calls of roughly equal cost. Purity gives
  independence; balance is your responsibility.
...
- With `LAWS.bend`/`PROOF.bend`: run `bend PROOF.bend`. Accept an unsafe-count
  success only when every unsafe annotation is intentional and documented.
...
- Import-sensitive change: add `bend file.bend --checkup`.
